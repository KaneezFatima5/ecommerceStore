...
class feedbacks
{
    ...
    function Selectfeedback($feedbackid){
        if($feedbackid==null){
            return $this->con->query("SELECT * FROM feedback inner join customers on feedback.CustomerID=customers.CustomerID inner JOIN product on Feedback.ProductID=product.ProductID");
        }
        else
        {
